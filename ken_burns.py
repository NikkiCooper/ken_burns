#!/usr/bin/env python3
#  ken_burns.py Copyright (c) 2026 Nikki Cooper
#
...
def print_filter_help():
    ...
    print()
    print(f"{bc.BOLD + bc.White_f}  Ken Burns  {bc.Magenta_f}─{bc.OKBLUE}  Image Filter Reference{bc.ENDC}")
    print(sep)
    ...
    para("Bilateral cost scales with d². light (~25 ops/pixel) is fast. "
         "strong (~169 ops/pixel) is ~6.8× heavier. On Ken Burns / drift slides "
         "it runs every frame; on static slides it runs once at load time.")
    ...
    para("Both filters are applied inside compute_slide_params() immediately "
         "after the GPU resize to 1920×1080. For non-drift/non-KB slides this is a "
         "one-time cost per slide. For Ken Burns and drift slides (--drift > 0) it is "
...
def print_fmap_help():
    ...
    print()
    print(f"{bc.BOLD + bc.White_f}  Ken Burns  {bc.Magenta_f}─{bc.OKBLUE}  Filter Map Reference{bc.ENDC}")
    print(sep)
    ...
    cmd("python ken_burns.py --img_path /path/to/imgs --gen_filter_map")
    print()
    ...
    print()
    cmd("python ken_burns.py --img_path /path/to/imgs --gen_filter_map --fmap_threshold 25")
    print()
    ...
    print()
    cmd("python ken_burns.py --img_path /path/to/imgs -o ~/out.mp4 --use_filter_map")
    print()
    ...
    print()
    cmd("python ken_burns.py --img_path /path --gen_filter_map --smooth subtle")
    para("  → flagged images get: {\"smooth\": \"subtle\", \"sharpen\": null}")
    print()
    cmd("python ken_burns.py --img_path /path --gen_filter_map --smooth light --sharpen light")
    ...
    cmd("python ken_burns.py --img_path /path --gen_filter_map --fmap_threshold 20")
    para("  → more images flagged (lower bar)")
    print()
    cmd("python ken_burns.py --img_path /path --gen_filter_map --fmap_threshold 45")
    ...
    cmd("python ken_burns.py --img_path ~/imgs/set_042 --gen_filter_map --smooth light")
    print()
    cmd("# Edit ~/imgs/set_042/filter_map.json if needed, then render")
    cmd("python ken_burns.py --img_path ~/imgs/set_042 -o ~/out/set_042.mp4 \\")
    cmd("    --duration 9 --drift 0.33 --rand_drift --use_filter_map")
    print()

    sub("Ken Burns mode with filter map")
    cmd("python ken_burns.py --img_path ~/imgs/set_042 -o ~/out/set_042.mp4 \\")
    cmd("    --ken_burns --ftb --duration 9 --use_filter_map --smooth subtle")
    ...
    cmd("python ken_burns.py --img_path ~/imgs/set_042 --gen_filter_map \\")
...
def apply_bilateral_smooth(gpu_img, smooth, intensity_override=None):
    """CUDA bilateral filter with intensity blend."""
...
def compute_slide_params(img, drift_value, direction, smooth=None, sharpen=None,
    ...
    """
    Compute crop geometry for one slide and pre-process on GPU.

    drift_value == 0.0: static hold — pre-resize once, store cpu_frame.
    drift_value  > 0.0: Ken Burns / drift zoom — store raw image for per-frame crop.
                        At drift_value=1.0 the crop change is ~15% (full Ken Burns).
    """
    h, w = img.shape[:2]
...
def _staircase_score(gray):
    ...
    for label in range(1, n_labels):
        ...
        if chain_len < 4:
            continue
        angles  = angle_map[ys, xs]
        doubled = 2.0 * angles
        mean_sin = np.sin(doubled).mean()
        mean_cos = np.cos(doubled).mean()
        circ_var = 1.0 - np.sqrt(mean_sin**2 + mean_cos**2)
...
def generate_filter_map(img_path, threshold, smooth=None, sharpen=None):
    ...
    print()
    print(f"{bc.BOLD + bc.White_f}  Ken Burns  {bc.Magenta_f}─{bc.OKBLUE}  Filter Map Generator{bc.ENDC}")
    print(sep)
...
# ---------------------------------------------------------------------------
# Ken Burns direction logic
# ---------------------------------------------------------------------------

def _ken_burns_directions(N, base_zoom_in):
    """
    Pre-compute per-slide zoom directions using streak-based alternation.
    Max 3 consecutive same direction; 70% chance to flip each slide.
    """
    directions = [base_zoom_in]
    current = base_zoom_in
    streak  = 1
    for _ in range(1, N):
        if streak >= 3:
            current = not current
...
def render_video(image_files, output_path, duration_sec, fps, trans_time_sec,
    ...
    Zoom modes:
      ken_burns=True          — full Ken Burns zoom (drift_value forced to 1.0),
                                streak-based alternating direction
      ken_burns=False, drift  — subtle configurable drift (0.0 = static hold)
    """
    bc = Bcolors()
    N           = len(image_files)
    ...
    # Determine per-slide zoom directions
    if ken_burns:
        drift_directions = _ken_burns_directions(N, base_zoom_in)
    elif rand_drift:
        half = N // 2
    ...
    def load_slide(idx):
        ...
        params = compute_slide_params(img, drift_value, direction,
                                      img_smooth, img_sharpen, smooth_intensity,
                                      target_w, target_h)
    ...
    try:
        for i in range(N):
            # Build per-slide line prefix (zoom direction for Ken Burns mode)
            if ken_burns:
                dir_col  = bc.Cyan_f if drift_directions[i] else bc.Magenta_f
                dir_str  = "zoom-in " if drift_directions[i] else "zoom-out"
                zoom_tag = f"  {bc.Dark_Gray_f}[{dir_col}{dir_str}{bc.ENDC}{bc.Dark_Gray_f}]{bc.ENDC}"
            else:
                zoom_tag = ""
            ...
            print(f"  {bc.Yellow_f}[{i + 1:3d}/{N}]{bc.Magenta_f}  "
                  f"{bc.OKBLUE}{os.path.basename(image_files[i])}{zoom_tag}{arrow}{bc.ENDC}")
            ...
            for f in range(frame_start, hold_frames):
                ...
                # ---- Opening / FTB incoming: fade-in from black ----
                ...
                # ---- FTB / closing outgoing: fade-out to black ----
                ...
                # ---- Crossfade outgoing: on-device GPU blend ----
...
def calculate_report(opts, img_path):
    ...
    drift      = 1.0 if ken_burns else opts.drift
    ...
    if ken_burns:
        dir_note = 'zoom-in' if opts.zoom_in else 'zoom-out'
        zoom_str = (f"{bc.OKBLUE}Ken Burns{bc.Dark_Gray_f}  "
                    f"({dir_note} first, streak-based alternation){bc.ENDC}")
    elif drift > 0:
        dir_note = (f"  {bc.Dark_Gray_f}random direction{bc.ENDC}"
                    if opts.rand_drift else
                    f"  {bc.Dark_Gray_f}alternating direction{bc.ENDC}")
        zoom_str = (f"{bc.OKBLUE}{drift:.2f}{bc.White_f}  "
                    f"{bc.Dark_Gray_f}(~{drift * 15:.1f}% crop change){bc.ENDC}{dir_note}")
    else:
        zoom_str = f"{bc.Dark_Gray_f}disabled{bc.ENDC}"

    script_name = "Ken Burns" if ken_burns else "Fade"
    sep = f"{bc.BOLD + bc.Dark_Gray_f}{'─' * 58}{bc.ENDC}"
    print()
    print(f"{bc.BOLD + bc.White_f}  {script_name}  {bc.Magenta_f}─{bc.OKBLUE}  Duration Calculator{bc.ENDC}")
    print(sep)
    ...
    print(f"  {bc.BOLD + bc.Magenta_f}Zoom             {bc.White_f}: {zoom_str}")
    print(sep)
...
if __name__ == "__main__":
    ...
    fps           = opts.fps
    trans_time    = opts.trans_time
    ken_burns_mode = opts.ken_burns
    base_zoom_in  = opts.zoom_in
    drift_value   = 1.0 if ken_burns_mode else opts.drift
    ...
    if ken_burns_mode:
        dir_str    = 'zoom-in' if base_zoom_in else 'zoom-out'
        zoom_label = (f"{bc.OKBLUE}Ken Burns{bc.Dark_Gray_f}  "
                      f"({dir_str} first, streak-based alternation){bc.ENDC}")
    elif drift_value > 0:
        ...
        zoom_label = f"{bc.OKBLUE}{drift_value:.2f}{dir_label}"
    else:
        zoom_label = f"{bc.Dark_Gray_f}disabled{bc.ENDC}"

    smooth          = opts.smooth
    smooth_intensity = opts.smooth_intensity
    ...
    if opts.use_filter_map:
        ...
        if not os.path.isfile(fmap_path):
            ...
            print(f"  {bc.OKBLUE}  python ken_burns.py --img_path {src_path} --gen_filter_map{bc.ENDC}\n")
            sys.exit(1)
    ...
    script_name = "Ken Burns" if ken_burns_mode else "Fade"
    sep = f"{bc.BOLD + bc.Dark_Gray_f}{'─' * 58}{bc.ENDC}"
    print()
    print(f"{bc.BOLD + bc.White_f}  {script_name}  {bc.Magenta_f}─{bc.OKBLUE}  Direct GPU Pipeline{bc.ENDC}")
    print(sep)
    ...
    print(f"  {bc.BOLD + bc.Magenta_f}FPS          {bc.White_f}: {bc.OKBLUE}{fps}{bc.ENDC}")
    print(f"  {bc.BOLD + bc.Magenta_f}Zoom         {bc.White_f}: {zoom_label}")
    ...
    trans_mode_str = 'random' if opts.random else ('ftb' if opts.ftb else 'crossfade')
    if ken_burns_mode:
        zoom_meta = f"ken_burns({'in' if base_zoom_in else 'out'})"
    elif drift_value > 0:
        zoom_meta = f"drift({drift_value:.2f},{'random' if opts.rand_drift else 'alt'})"
    else:
        zoom_meta = 'off'
    ...
    ok = render_video(
        image_files, dest_vid, duration_sec, fps, trans_time,
        transition_types, drift_value, opts.rand_drift, use_hevc,
        smooth, sharpen, smooth_intensity, filter_map_data,
        ken_burns_mode, base_zoom_in, metadata_comment
    )
