#  cliOpts.py Copyright (c) 2026 Nikki Cooper
#
...
def cliOpts():
    ...
    parser = argparse.ArgumentParser(
        description="Ken Burns: GPU-accelerated slideshow — Ken Burns zoom, crossfade dissolve, or fade-to-black",
    ...
    grp_time.add_argument("--trans_time", action="store", type=float, dest="trans_time", default=1.0,
                          help=f"Transition duration in seconds\n"
                               ...
                               f"  --ken_burns : fade-to-black duration per side (when --ftb) or dissolve overlap\n"
                               f"{bc.BOLD+bc.Magenta_f}Default: {bc.OKBLUE}1.0s{bc.ENDC}")

    # -------------------------------------------------------------------------
    # Ken Burns
    # -------------------------------------------------------------------------
    grp_kb = parser.add_argument_group("Ken Burns")
    grp_kb.add_argument("--ken_burns", action="store_true", dest="ken_burns", default=False,
                        help=f"Enable full Ken Burns zoom effect\n"
                             f"  Each slide zooms in or out across the full frame (≈15%% crop change)\n"
                             f"  Direction alternates with streak-based randomisation:\n"
                             f"    70%% chance to flip direction each slide, max 3 same direction in a row\n"
                             f"  Mutually exclusive with --drift\n"
                             f"  Combine with any transition: --ftb, crossfade (default), --random\n"
                             f"{bc.BOLD+bc.Magenta_f}Default: {bc.OKBLUE}off{bc.ENDC}")
    grp_kb.add_argument("-z", "--zoom_in", action="store_true", dest="zoom_in", default=False,
                        help=f"Start first slide zooming in  (Ken Burns mode only)\n"
                             f"  Without this flag the first slide zooms out\n"
                             f"  Has no effect unless --ken_burns is set\n"
    ...
    grp_trans.add_argument("--drift", action="store", type=float, dest="drift", default=0.0,
                           help=f"Subtle per-slide zoom drift amount  (mutually exclusive with --ken_burns)\n"
    ...
    # ---- filter_help / fmap_help: no other args needed ----
    if options.filter_help:
        from ken_burns import print_filter_help
        print_filter_help()
        sys.exit(0)

    if options.fmap_help:
        from ken_burns import print_fmap_help
        print_fmap_help()
        sys.exit(0)
    ...
    # ---- gen_filter_map / calculate: only need --img_path ----
    if options.gen_filter_map or options.calculate:
    ...
    if options.ken_burns and options.drift != 0.0:
        print(f"{bc.BOLD+bc.Red_f}--ken_burns and --drift are mutually exclusive.{bc.ENDC}")
        sys.exit(1)

    # ---- warnings for no-op flags ----
    if options.zoom_in and not options.ken_burns:
        print(f"{bc.BOLD+bc.Yellow_f}Warning: --zoom_in has no effect without --ken_burns{bc.ENDC}")

    if options.rand_drift and options.drift == 0.0 and not options.ken_burns:
    ...
    if options.smooth_intensity is not None:
        if not (0.0 <= options.smooth_intensity <= 100.0):
            print(f"{bc.BOLD+bc.Red_f}--smooth_intensity must be in range 0–100 "
                  f"(got {options.smooth_intensity}){bc.ENDC}")
            sys.exit(1)
